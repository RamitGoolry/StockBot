...
from trading_env import Portfolio, TradingEnv
from trading_env import INVALID_BUY, VALID_BUY, INVALID_SELL, PROFIT, LOSS, HELD
from trading_env import BUY, SELL, HOLD
...
def test_trading_env_buy_securities():
    ...
    te.trade(BUY, 'BTC', ONE_BITCOIN_PRICE)
...
def test_trading_env_hold():
    te = TradingEnv(Portfolio(100), TEST_DATA_SOURCE)
    reward = te.trade(HOLD)

    assert reward[0] == HELD
